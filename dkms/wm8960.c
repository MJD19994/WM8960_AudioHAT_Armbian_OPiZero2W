...
static const struct snd_soc_dapm_route audio_paths[] = {
	...
	/* Mic bias routes - ensure MICB powers on during capture */
...
static int wm8960_configure_clocking(struct snd_soc_component *component)
{
	...
	if (!(iface1 & (1 << 6)) && !wm8960->sysclk) {
		...
		// return 0; /* PLL fix: proceed with clock config in slave mode */
	}
	...
	} else if (wm8960->clk_id == WM8960_SYSCLK_PLL) {
		/* PLL mode: configure_pll will calculate output freq */
		freq_out = 0;
	} else {
					...
					unsigned int freq, int dir)
{
	...
	/* Force PLL mode - onboard 24MHz crystal, no clock driver */
...
static int wm8960_i2c_probe(struct i2c_client *i2c)
{
	...
	/* Boot in PLL mode for onboard crystal */
